...
###########################################################
resource "aws_iam_saml_provider" "pn_vpn" {
...
resource "aws_ec2_client_vpn_endpoint" "vpn" {
  ...
  client_cidr_block      = var.vpc_pn_vpn_cidr

  vpc_id             = module.vpc_pn_vpn["enabled"].vpc_id
  security_group_ids = [aws_security_group.vpc_pn_vpn_clients.id]
  ...
  authentication_options {
    ...
    saml_provider_arn = aws_iam_saml_provider.pn_vpn[0].arn
  }

  connection_log_options {
    enabled              = true
    cloudwatch_log_group = aws_cloudwatch_log_group.pn_vpn[0].name
  }
